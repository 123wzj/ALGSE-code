"""
THEIA E3 parser: entity and event extraction, with counts.
...
def store_file(file_path,writefile,count_subject):
    ...
    for i in file_uuid2path.keys():
        if file_uuid2path[i]!='none':
            datalist.append(str(j)+": "+file_uuid2path[i])
...
def store_event(file_path, subjectID, fileID, netID,writeFile):
    ...
    with open(writeFile, 'w',encoding='utf-8') as wf:
        ...
        # wf.writelines(str(total_event_count))
...
if __name__ == '__main__':
    ...
    # groundtruthPath = "ProcessedData/drapra3/theia/evaluation/theia_groundtruth_uuid.txt"
    # groundtruthUuidPath = "ProcessedData/drapra3/theia/evaluation/theia_groundtruth_nodeId.txt"
